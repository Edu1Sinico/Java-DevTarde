...
public class SerializacaoConsulta {
    // Método para serializar uma lista de usuários em um arquivo binário
    public static void serializar(String arquivo, List<AgendarConsulta> agendarConsultaList) {
        try (ObjectOutputStream outputStream = new ObjectOutputStream(new FileOutputStream(arquivo))) {
            ...
            outputStream.writeObject(agendarConsultaList);
    ...
    public static List<AgendarConsulta> deserializar(String arquivo) {
        try {
            ...
            return (List<AgendarConsulta>) inputStream.readObject();
